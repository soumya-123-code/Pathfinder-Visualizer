...
import { resetGird } from "../utils/resetGrid";
...
export function Nav({ isVisualizationRunningRef }: { isVisualizationRunningRef: MutableRefObject<boolean> }) {
    ...
    const handleGenerateMaze = (maze: MazeType) => {
        if (maze === 'NONE') {
            setMaze(maze)
            resetGird({ grid, startTile, endTile });
            return;
        }
    ...
    const handlerRunVisualizer = () => {
        if (isGraphVisualized) {
            setIsGraphVisualized(false);
            resetGird({ grid: grid.slice(), startTile, endTile });
            return;
        }
        ...
        <div className="flex items-center justify-center min-h-[4.5rem] border-b shadow-gray-600 sm:px-5 px-0">
            <div className="flex items-center lg:justify-between justify-center w-full sm:w-[52rem]">
                ...
                <div className="flex sm:items-end items-center justify-start sm:justify-between sm:flex-row flex-col sm:space-y-0 space-y-3 sm:py-0 py-4 sm:space-x-4">
                    ...
                    <Select label="Speed" value={speed} options={SPEEDS} onChange={(e) => {
                    ...
                    <PlayButton
                        ...
                        handlerRunVisiualizer={handlerRunVisualizer}
                    />
                </div>
            </div>
        </div>
